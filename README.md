...
すでにタイトルと日付がそろって載っている配信は、yt-dlp に聞き直さずに
そのまま使います。問い合わせるのは、新しく足したものと、前回取れなかった
項目が残っているものだけです。全部取り直したいときは `-Refresh` を付けます。

yt-dlp が失敗したときは、画面に理由が出ます（「YouTubeのボット確認で
止められました」など）。そのときも次のように動きます。

- 載っていたタイトルと日付は消しません（`-Refresh` のときも同じ）
- 新しい配信は、埋め込み用の oEmbed からタイトルだけ取ります。日付は
  空欄のまま進み、yt-dlp が動くときにもう一度実行すると埋まります
- どこからも取れなかったときだけ、タイトルの代わりに動画IDが入ります

yt-dlp が使えない環境では、`public/index.html` の `ARCHIVES:BEGIN` 〜
`ARCHIVES:END` の間を直接編集しても構いません。`title` を省略すると
動画IDが表示されます。手で直したタイトルと日付は、次にスクリプトを
実行しても引き継がれます（`-Refresh` を付けたときは、取り直せた値で
上書きします。`note` などそれ以外の書き足しは消えます）。
...
RSSフィードは、有効なチャンネルにもときどき 404 や 500 を返します。
そこで諦めると、配信中でも「いまは配信していません」のままになるので、
動画IDは次の順に試して、最初に取れたものを使います。

- RSSフィード
- もう一度RSSフィード（一時的な失敗なら、これで通ることが多いです）
- `playlistItems.list` でアップロード再生リストを読む（チャンネルIDの
  `UC` を `UU` に変えたID。RSSとは別の入口で、**1ユニット**）

Cloudflare に寝かせてもらうのは、成功した答えだけです。失敗まで60秒
寝かせると、読み直しても同じ失敗が返ってくるためです。

クォータは1日10,000ユニットの無料枠に対し、60秒キャッシュなので**最大でも
1日約1,440ユニット**です。RSSが丸1日落ちて毎回 `playlistItems.list` に
回っても約2,880ユニットです。超えてもAPIが403を返すだけで、その答えは
...
APIの経路（`YT_API_KEY` あり）では、答えの中身が次のようになります。

| 項目 | 見どころ |
| --- | --- |
| `attempts` | 試した順と結果。`["feed 200"]` なら正常、`["feed 404", "feed 200"]` なら読み直しで通った |
| `idSource` | 動画IDをどこから取れたか（`feed` / `feed-retry` / `playlist`）。`null` ならどこからも取れていない |
| `apiStatus` / `apiError` | `videos.list` の status と、失敗したときの理由（403 ならクォータ切れなど） |
| `decided` | 判定結果 |

控えの `playlistItems.list` はRSSが落ちたときしか通らないので、
`/live-now?debug=1&skipfeed=1` で、RSSを飛ばしてこちらだけを試せます
（1ユニット余分にかかります）。`attempts` が `["playlist 200"]` になれば、
控えの経路も動いています。
